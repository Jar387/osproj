...
bst_node_t *
bst_insert(bst_node_t ** root, void *data, unsigned int id)
{
	if (*root == NULL) {
	...
	(*root)->height =
	    MAX(height((*root)->child1), height((*root)->child2)) + 1;
...
static bst_node_t *
min_value_node(bst_node_t * node)
{
...
static bst_node_t *
__bst_delete(bst_node_t ** root, unsigned int id)
{
	if (*root == NULL)
		return *root;
	...
	if (id < (*root)->id) {
		...
		if ((*root)->child1 == NULL) {
			...
			*root = temp;
			...
			(*root)->data = temp->data;
			(*root)->child2 =
			    bst_delete(&(*root)->child2, temp->id);
		}
	}
...
static bst_node_t *
__bst_search(bst_node_t * root, unsigned int id)
{
	while (root != NULL) {
		if (id < root->id) {
			root = root->child1;
